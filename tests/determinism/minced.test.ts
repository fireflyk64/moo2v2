// MincedOnion viability gate: the reactive rival-reading brain
// (mincedOnion.ts) must play full games against the onion brain without
// stalling and develop a real empire. Same shape as onion.test.ts — the fast
// regression gate the tournament improvement loop leans on; the score floor
// is deliberately permissive — cross-brain BALANCE is judged by the
// tournament's score tables, not here (win counts at N=2 are coin flips).
...
function identity(name: string) {
  return { name, engineVersion: '0.1.0', dataVersion: 'dv-test', roomCode: 'MINCE', lobbyServer: 'memory' };
}
...
describe('MincedOnion viability', () => {
  it(
    'minced vs onion plays full games from both seats and develops',
    async () => {
      const a = await match(SEED, 'minced', 'onion');
      const b = await match(SEED, 'onion', 'minced');
      ...
      // the minced empire is alive and building something real by t110
      for (const minced of [a.d0, b.d1]) {
        expect(minced.score).toBeGreaterThan(0);
        expect(minced.colonies).toBeGreaterThanOrEqual(2);
        expect(minced.apps).toBeGreaterThanOrEqual(8);
      }
      // permissive cross-brain floor: catches a collapsed minced economy
      // without turning seed noise into flakes
      const mincedTotal = a.d0.score + b.d1.score;
      const onionTotal = a.d1.score + b.d0.score;
      expect(mincedTotal).toBeGreaterThanOrEqual(Math.floor(onionTotal * 0.5));
    },
    600_000,
  );
});
