...
- [ ] Have a single player bot mode with a very simple bot. the bot copies player ship designs and is granted the nearest colony every time the player expands and the same research whenever the player researches. It sets all planets to have enough food then 1 researcher and the rest on industry, so it has a chance to research more. it builds random buildings and ships with its planets. It randomly uses surplus money to buy things. If it runs out of money it is granted 100 BC. when player clicks aggressive mode it sends half its fleet to 2 random planets that are as close to the player as possible preferably owned by the player. it is unaffected by food. All of the bonuses should not affect game simulation but should be handled in the AI functions to avoid special cases. so the ai function recognizes that an opponent has a technology and grants it to itself or the ai function recognizes the bot has 0 credits and grants 100. That way the game simulation does not special case bots or production or anything and the bot ai function is the messy one. In this single player mode, the lobbylink is not necessary and  
