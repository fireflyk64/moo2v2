...
### 2. Ship designs never check the weapon is researched — turn‑1 monster/endgame weapons  [VERIFIED]
`src/engine/shipdesign.ts:212‑320` (`fitWeapon`/`designStats`), shared P2P validator at `src/engine/commands.ts:465`

`designStats` gates computer tier, shield tier, specials (`knownApps.includes`), and hull —
but there is **no check that `empire.knownApps` contains the weapon itself** (only weapon
*mods* are gated, and the `techId === 0` guard only covers `classId 3|5`). On a fresh
turn‑1 empire, `save_design` was confirmed to ACCEPT designs mounting `zeon_missile`,
`dragon_breath` (300‑dmg auto‑hit monster torpedo), and `mauler_device` with `knownApps`
containing none of them. Since this is a host‑validated multiplayer game, a modified client
can field 300‑damage dragon‑breath frigates on turn 1 and one‑shot everyone; every peer's
validator accepts it. **Fix:** in `designStats`, reject any weapon whose granting
application is not in `empire.knownApps`.
...
### 21. The "Depression" random event pays money to empires already in debt  [VERIFIED]
`src/engine/npc.ts:388‑389`

```js
const loss = Math.floor(empire.bc / 5); empire.bc -= loss;
```

With a negative treasury (an expected state — deficits, CP overage, salaries all drive `bc`
below 0), `Math.floor(-100/5) = -20` and `bc -= -20` **adds** 20 BC. Verified: bc −100 →
−80 on the depression event. A "bad" event that lucky races are protected from is a bailout
for anyone in debt, while a healthy empire loses 20 %. **Fix:** clamp `loss` at ≥ 0.
...
### 28. New colonies are seeded with a farmer even on worlds where farming is impossible  [VERIFIED] [KNOWN‑UNFIXED residue — bugs.md "farmer on barren"]
`src/engine/commands.ts:352` (`applyColonize`), `src/engine/pipeline.ts:312` (`colony_base`)

Player job commands are correctly blocked on non‑farmable worlds (`validateSetJobs`), but new
colonies are auto‑seeded `farmers: 1` unconditionally. Colonizing a barren/hostile planet
starts with the sole colonist farming for 0 food — exactly the "wasted farmer" trap the
barren‑farming gate was built to prevent. **Fix:** seed the starting colonist as a worker
(or a scientist) when `!farmingViable`.

---

## P3 — UI / UX / cosmetic / doc drift
...
### 30. Picking an already‑queued repeatable item in the Build dropdown silently deletes its duplicates  [VERIFIED]
`src/ui/screens/Spreadsheet.svelte:279‑283`

```js
const items = row.queue.includes(item) ? [item, ...row.queue.filter(x => x !== item)] : …
```

Ships/projects are legitimately repeatable. With queue `[star_base, design:5, design:5,
design:5]`, selecting that warship in the Build column to promote it produces
`[design:5, star_base]` — two of the three queued ships silently vanish. Verified in a live
session (`[freighter_fleet, star_base]` → selecting the queued item reordered/removed as
described). **Fix:** only de‑duplicate buildings; preserve repeat counts for ships/projects.
...
### 33. Colonize / outpost buttons appear at monster‑guarded systems but clicking them is a silent no‑op  [CODE]
`src/engine/selectors.ts:550` (`settleTargets` doesn't check `hostileMonsterAt`) vs `src/engine/commands.ts:336`

`settleTargets` offers the colonize/outpost action at guarded systems; MapView submits without
checking `res.error`, so the button does nothing with zero feedback (Fleets shows the error
for the same action — inconsistent). **Fix:** hide the action at guarded systems, or surface
the rejection.
...
### 35. Entering the Battle Lab mid‑game unmounts the whole GameShell → battle prompts unreachable  [VERIFIED]
`src/ui/App.svelte:47‑57` routes `#battle-lab` above the game screen; `Empires.svelte`/`Designer.svelte` set the hash mid‑game

Verified live: after clicking "⚗ Simulate with this game's ships", the commit button and the
entire top bar are gone (`commit visible=false, turn visible=false`). If other players commit
and a battle starts while you're in the lab, the orders dialog/commit bar/auto‑turn banner are
all unmounted and your orders default after the 60 s timeout (finding 5) with no visible
prompt. **Fix:** render the lab as an overlay above the running game, or keep the commit bar
mounted.

### 36. Leaving a game leaks its reports, replays, and an open battle viewer into the next game  [CODE]
`src/ui/state.svelte.ts:109‑126` — `leaveGame` resets only screen/chat/version

`app.reports`, `app.replays`, and `app.viewing` persist across `leaveGame`. Start a new solo
game (or join another room) in the same tab and the Reports badge shows the old game's count,
old turn events and battle replays appear under the new game's turns, the "⚔ N new battles"
badge fires from the previous game, and a viewer left open reopens over the new game.
**Fix:** clear reports/replays/viewing in `leaveGame`.
...
### 40. Armed scouts fight in battles but cannot block an invasion landing or a blockade  [VERIFIED]
`src/engine/ground.ts:57,191` count only `shipKind === 'design'` vs `battles.ts:43` `isWarship` includes scouts

Since scouts were given a laser and made combat units, they fight in battles — but
`resolveInvasions`/`isBlockaded` still only count `design` ships, and when the attacker brings
only transports no battle is even detected. Verified: a defender with an armed scout in orbit
lost the colony to two transports without a shot. A player picketing a border colony with a
scout is not actually contesting landings. **Fix:** treat armed scouts as warships in the
invasion/blockade checks too.
...
### 44. designDps overstates beam DPS (~20 %+) and ignores the pd/hv damage multipliers  [VERIFIED]
`src/engine/combat.ts:540‑543` (fires every `cd+1` ticks) vs `:884` (`designDps` divides by `cd`)

Firing sets `cds = cd` and uses a decrement‑then‑fire pattern, so the real period is `cd+1`
ticks (measured: a laser fires on ticks 0, 6, 12… with cd 5), but `designDps` divides by `cd`,
overstating sustained beam DPS ~20 % (more for short cooldowns). It also omits the sim's
`pd` ×0.5 and `hv` ×1.5 multipliers, so a pd‑modded gun shows double its real short‑range DPS.
**Fix:** divide by `cd+1` and apply the pd/hv multipliers in the readout.
...
### 47. Antaran raids resolve against the lowest‑id empire sharing the target star, not the raided empire  [CODE]
`src/engine/battles.ts:76‑78`

`antaranUpkeep` aims a raid at the largest empire's most populous colony, but if two empires
hold colonies at that star the battle defender — and the colony razed on an Antaran win — is
whichever owner has the lower empire id. Empire 0's small colony can fight and eat the raze
meant for empire 3's capital in the same system. **Fix:** carry the intended target empire on
the pending raid.

### 48. Accepted diplomacy proposals produce no events, and a rejected‑by‑`acceptProposal` accept silently no‑ops  [CODE]
`src/engine/commands.ts:740` calls `acceptProposal`/`executeSurrender` but discards their events and error
...
## bugs.md items confirmed FIXED (not re‑reported)

For the record, these earlier reports were checked in code and found addressed: ETA "1t but
takes multiple turns" (movement ETA is now consistent), no wormholes on homeworld systems,
no size‑1 planets, empty‑star fraction bounded, pick‑points selector (10/14), MIRV now raises
DPS and quadruples warheads, MIRV/ECCM/point‑defense gated deeper in the tech tree, mauler
always hits, doom stars render round, retreat exits via the nearest map edge (real battles),
starlight projector has a flashy render, replays are emitted to participants only, monsters
now guard prize systems (55 % vs 8 %), the auto‑turn timer advances one turn at a time (the
"jumped 60 turns" report), cache‑busting build id + COI service worker, the *local* save‑note
auto‑clears (but the PBM one does not — finding 59), outposts filtered from the colonies
table, colonist‑drop failures now show a ⛔ note, the "fleets travel star‑to‑star" tip is
once‑per‑game, and `andromedan`/`Andromedan` naming replaced `antaran` in the player‑facing
text.

Two earlier reports I initially thought fixed are in fact **still broken** (see the
protocol section): the "host offline" warning still shows while the host is online and never
clears (finding 58), and the PBM "uploaded" note sticks around for the whole session
(finding 59).
...
### 55. `hello` never checks `engineVersion` → players on different engine builds are allowed to join and silently diverge  [VERIFIED] — P1
`src/protocol/host.ts:256‑268` (the field is on the wire at `messages.ts:58`)

`onHello` rejects mismatched `protocolVersion` and `dataVersion` but never reads
`engineVersion` — which is bumped independently for logic changes (e.g. the starvation
floor). `docs/save-compatibility.md` says cross-version *joining* must be refused, and the
local-resume path checks it, so the omission is clearly unintended. Verified: a client
helloing with `engineVersion:'9.9.9'` is welcomed and seated. After an engine-only patch,
an updated player joins an un-updated host, the game starts fine, then every turn
"desyncs" — and via finding 54(c) the recovery **loops forever** instead of surfacing "your
version differs." **Fix:** reject on `engineVersion` mismatch in `onHello`.
...
### 57. The sealed-bid pick auction resolves race configs without the game's pick budget → 11–14-point races are silently replaced by the Solari preset  [VERIFIED] — P1
`src/protocol/auction.ts:31‑33, 36‑51, 92, 113‑119` vs `src/engine/adapter.ts:34‑52`

`resolvedPicks` calls `resolveRaceConfig(json)` with **no budget argument**, so it defaults
to 10; `resolveRaceConfig` falls back to the Solari preset when `validatePicks(picks, 10)`
fails. The game supports `pickPoints: 14`, but `findContested`, `budgetSlack`, and
`resolveAuction`'s final rewrite all resolve unbudgeted. Verified with real data: a valid
14-point race (`dictatorship + lithovore + farming2`) is resolved to the Solari preset;
two players with fully **disjoint** 14-point races are reported as contesting picks (both
fell back to the same preset) so an auction fires over picks nobody shares; and
`resolveAuction` rewrites both players' `raceJson` to preset picks, recorded permanently in
`game_start`. In-game: turn on "Pick bidding" with a 14-point budget and everyone's custom
race is silently swapped for the default at game start. (This is the engine-integration
half of the `bugs.md` "we need a picks = 14" request — the setting exists but the auction
path ignores it.) **Fix:** thread `settings.pickPoints` through every `auction.ts` resolve
call. Related lower-severity auction issues the deep-dive traced: race configs aren't
frozen during the 45 s bid window (a bidder can drop a contested pick after
`auction_begin` reveals who holds what, forcing an honest co-holder to pay/lose), and a
re-entrant `startGame` mid-auction silently cancels the auction and starts with contested
picks intact (`host.ts:312‑320, 402‑419`).
...
### 63. Two minor PBM-server durability nits  [CODE] — P3
`server/pbm/pbm.go:303‑349, 327‑343, 495‑505`

New-room creation computes `isNew` via `os.Stat` outside the lock mutex, so two concurrent
first-uploads for the same code both claim the creator lock (last-writer-wins; atomic
rename prevents corruption). A save also renames `save.moo2save` into place *before*
writing `meta.json`, so a crash between the two leaves a new save with stale meta.
`writeSecrets` is a single non-atomic write (a crash mid-write drops seat protections). All
low-impact and self-correcting on the next successful save. **Fix:** stat-and-claim under
`s.mu`; write meta first (or both to temp then rename); temp+rename secrets too.

**Confirmed sound (not re-reported):** command-turn validation, battle-order ownership
checks, the resync tail service, gzip/save-envelope integrity checks, the Go server's lock
mutex/TTL logic, seat-protection hashing, constant-time token comparisons, and path
traversal guards all check out (`go test ./...` passes). The auto-turn timer advances
exactly one turn (the "jumped 60 turns" report is fixed and locked by
`tests/protocol/autoturn.test.ts`), with only a re-arm nit: `armAutoTurn` resets its full
deadline on every `commit_status`, so a committed player re-sending `commit_turn` can
postpone the laggard cutoff indefinitely.
